...
export default function Leads() {
  ...
  const save = async (e) => {
    e.preventDefault()
    toast('Lead saving disabled (no backend yet)', 'error')
  }

  const doDelete = async () => {
    toast('Lead deletion disabled (no backend yet)', 'error')
  }
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      <PageHeader
        title="Leads"
        subtitle={`${rows.length} lead${rows.length !== 1 ? 's' : ''} total`}
      ...
      <div className="card">
        ...
        <Table
          ...
          data={rows}
          loading={loading}
          emptyMessage="No leads yet — create your first!"
        />
      </div>
        ...
        <form onSubmit={save} className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className="input" value={form.phone} onChange={set('phone')} placeholder="+1 555 123 4567" />
            </div>

            <div>
              <label className="label">Source</label>
              <input className="input" value={form.source} onChange={set('source')} placeholder="Instagram" />
            </div>
      ...
      <ConfirmDialog
        ...
        loading={deleting}
        title="Delete Lead"
        message="This will permanently delete the lead. This action cannot be undone."
      />
    </div>
  )
}

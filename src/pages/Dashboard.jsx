...
import { Users, FileText, Target, TrendingUp, Calendar, Zap } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const platformData = data
    ? Object.entries(data.breakdowns?.postsByPlatform || {}).map(([name, value]) => ({ name, value }))
    : []
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 stagger">
        <StatCard label="Total Clients"   value={loading ? '—' : formatNumber(ov.totalClients)}   icon={Users}      color="brand"   sub={`${ov.activeClients ?? 0} active`} />
        <StatCard label="Posts Published" value={loading ? '—' : formatNumber(ov.publishedPosts)} icon={FileText}   color="emerald" sub={`${ov.scheduledPosts ?? 0} scheduled`} />
        <StatCard label="Total Leads"     value={loading ? '—' : formatNumber(ov.totalLeads)}     icon={Target}     color="amber"   sub={`${ov.newLeads ?? 0} new`} />
        <StatCard label="Conversion Rate" value={loading ? '—' : `${ov.conversionRate ?? 0}%`}   icon={TrendingUp} color="violet"  sub={formatCurrency(ov.totalLeadValue)} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={data?.monthlyTrend || []} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="count" stroke="#6366f1" strokeWidth={2} fill="url(#grad)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className="card p-5">
          ...
          {loading ? (
            ...
            <div className="h-48 flex items-center justify-center text-slate-500 text-sm">No posts yet</div>
          ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <RecentList
          ...
          loading={loading}
          renderItem={(c) => (
            <div key={c.id} className="flex items-center justify-between py-2.5 border-b border-bg-border last:border-0">
              <div>
        ...
        <RecentList
          ...
          loading={loading}
          renderItem={(p) => (
            <div key={p.id} className="flex items-center justify-between py-2.5 border-b border-bg-border last:border-0">
        ...
        <RecentList
          ...
          loading={loading}
          renderItem={(l) => (
            <div key={l.id} className="flex items-center justify-between py-2.5 border-b border-bg-border last:border-0">
              <div>
